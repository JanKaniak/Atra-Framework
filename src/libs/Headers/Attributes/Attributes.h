...
{
private:
    AttributeDescriptions *attributeDescs_;
    std::vector<std::unique_ptr<Attribute>> attributes_;
    AttributeFactory attributeFactory_;

public:
    Attributes() {};
    Attributes(AttributeDescriptions *attributeDescs);

    void addAttribute(std::string name, AttributeType type, AttributeTypeVariant minimum, AttributeTypeVariant maximum);
